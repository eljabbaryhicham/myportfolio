...
export function deriveCloudinarySpriteVtt(sourceUrl: string | null | undefined): string | null {
  ...
  // Rebuild with a single `fl_sprite` flag and the `.vtt` extension. Cloudinary's
  // seek-thumbnail sprite tiles are fixed at 160x90 and cannot be raised via
  // width/quality qualifiers (a width qualifier just downscales the whole sprite,
  // making tiles blurrier). So we keep the plain flag for the sharpest option.
