...
| 2026-08-23 | FIX: "Add from URL" media upload crashed in production ("docs/service-account.json is missing") — `initializeServerApp` now resolves credentials in order: FIREBASE_SERVICE_ACCOUNT_KEY env var (JSON, \n-unescaped) → optional local docs/service-account.json → Application Default Credentials (works natively on Firebase App Hosting / Cloud Run). No file or env var required on deployed hosting | ✅ |
